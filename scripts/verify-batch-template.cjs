#!/usr/bin/env node
/**
 * Batch Audio Verifier Template
 * Verifies a specific batch of audio files
 */
...
// Batch configuration - to be customized for each batch
const BATCH_CONFIG = {
  name: 'BATCH_NAME',
  description: 'BATCH_DESCRIPTION',
  // List of audio IDs that must exist
  requiredIds: [],
...
async function verifyBatch() {
  ...
  // Save report
  const reportPath = path.join(REVIEW_DIR, `${BATCH_CONFIG.name.toLowerCase().replace(/\s+/g, '-')}-verification.json`);
...
// Allow config to be passed from command line or use default
if (process.argv[2]) {
  const configPath = path.join(process.cwd(), process.argv[2]);
  if (fs.existsSync(configPath)) {
    const customConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    Object.assign(BATCH_CONFIG, customConfig);
  }
}
